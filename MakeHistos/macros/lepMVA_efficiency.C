		...
		if (muon_tag.pt > 30 and muon_tag.relIso < 0.12 and MuonID(muon_tag, "tight") and MuonTrig(muon_tag, obj_sel.year)) { //non-prompt
	            if( muon_probe.pt > 10 and muon_probe.pt < 20) {
	                BookAndFill( h_map_1D, h_pre+"mu_pt_10_20_all", 				16,-2.4,2.4, muon_probe.eta, event_wgt);
	              	if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_10_20_tight",   16,-2.4,2.4, muon_probe.eta, event_wgt);	
	            }
	            else if( muon_probe.pt < 30) {
	                BookAndFill( h_map_1D, h_pre+"mu_pt_20_30_all",					16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_20_30_tight",	16,-2.4,2.4, muon_probe.eta, event_wgt);
	            }
	            else if( muon_probe.pt < 40) {
                        BookAndFill( h_map_1D, h_pre+"mu_pt_30_40_all",                                 16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_30_40_tight",   16,-2.4,2.4, muon_probe.eta, event_wgt);
                    }
	            else if( muon_probe.pt < 50) {
                        BookAndFill( h_map_1D, h_pre+"mu_pt_40_50_all",                                 16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_40_50_tight",   16,-2.4,2.4, muon_probe.eta, event_wgt);
                    }
	            else if( muon_probe.pt < 100) {
                        BookAndFill( h_map_1D, h_pre+"mu_pt_50_100_all",                                16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_50_100_tight",  16,-2.4,2.4, muon_probe.eta, event_wgt);
                    }
	            else if( muon_probe.pt < 200) {
                        BookAndFill( h_map_1D, h_pre+"mu_pt_100_200_all",                               16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_100_200_tight", 16,-2.4,2.4, muon_probe.eta, event_wgt);
                    }
	            else {
                        BookAndFill( h_map_1D, h_pre+"mu_pt_200_more_all",                              16,-2.4,2.4, muon_probe.eta, event_wgt);
                        if( muon_probe.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"mu_pt_200_more_tight",16,-2.4,2.4, muon_probe.eta, event_wgt);
                    }
		}
	      }
	    }
...
//	      if ( muon.relIso < 0.12 and MuonID(muon, "tight") and muon.charge + ele.charge == 0 and br.nBMed == 2) { // prompt
	      if ( muon.relIso < 0.12 and MuonID(muon, "tight") and muon.charge + ele.charge != 0 and mu_ele.M() > 12 ) { // non-prompt
                if( ele.pt > 10 and ele.pt < 20) {
                    ...
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_10_20_tight",         16,-2.4,2.4, ele.eta, event_wgt);
                }
                else if( ele.pt < 30) {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_20_30_all",                                16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_20_30_tight",         16,-2.4,2.4, ele.eta, event_wgt);
                }
                else if( ele.pt < 40) {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_30_40_all",                                16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_30_40_tight",         16,-2.4,2.4, ele.eta, event_wgt);
                }
                else if( ele.pt < 50) {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_40_50_all",                                16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_40_50_tight",         16,-2.4,2.4, ele.eta, event_wgt);
                }
                else if( ele.pt < 100) {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_50_100_all",                               16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_50_100_tight",        16,-2.4,2.4, ele.eta, event_wgt);
                }
                else if( ele.pt < 200) {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_100_200_all",                              16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_100_200_tight",       16,-2.4,2.4, ele.eta, event_wgt);
                }
                else {
                    BookAndFill( h_map_1D, h_pre+"ele_pt_200_more_all",                             16,-2.4,2.4, ele.eta, event_wgt);
                    if( ele.lepMVA > 0.8) BookAndFill(h_map_1D, h_pre+"ele_pt_200_more_tight",      16,-2.4,2.4, ele.eta, event_wgt);
                }
	      }
	  }
