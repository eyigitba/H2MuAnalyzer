...
void GenRecoPtDiffVsD0VsPt( TString sample = "", TString in_dir = "", TString out_dir = "",
  ...
  for (UInt_t iEvt = 0; iEvt < in_chain->GetEntries(); iEvt++) {
	...
	for (int iOpt = 0; iOpt < OPT_CUTS.size(); iOpt++) {
	  ...
	  BookAndFill( "h_"+OPT_CUTS.at(iOpt)+"_d0", 10000, -500,  500, 10000*mu.d0_PV*mu.charge );
	  BookAndFill( "h_"+OPT_CUTS.at(iOpt)+"_pt", 10000,    0, 1000, mu.pt );
	  ...
	  for (int iCat = 0; iCat < CAT_CUTS.size(); iCat++) {
	    ...
	    BookAndFill( h_pre+"d0", 10000, -500,  500, 10000*mu.d0_PV*mu.charge );
	    BookAndFill( h_pre+"pt", 10000,    0, 1000, mu.pt );
	    
	    BookAndFill( h_pre+"dPt_Roch_vs_d0",       1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge,        mu.pt_Roch - gen.pt );
	    BookAndFill( h_pre+"dPt_PF_vs_d0",         1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge,        mu.pt      - gen.pt );
	    BookAndFill( h_pre+"dRelPt_Roch_vs_d0",    1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge,   100*(mu.pt_Roch - gen.pt) / gen.pt );
	    BookAndFill( h_pre+"dRelPt_PF_vs_d0",      1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge,   100*(mu.pt      - gen.pt) / gen.pt );

	    BookAndFill( h_pre+"dRelPt1p4_Roch_vs_d0", 1000, -50, 50, 400, -64, 64, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 1.4) );
	    BookAndFill( h_pre+"dRelPt1p4_PF_vs_d0",   1000, -50, 50, 400, -64, 64, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 1.4) );
	    BookAndFill( h_pre+"dRelPt1p6_Roch_vs_d0", 1000, -50, 50, 400, -32, 32, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 1.6) );
	    BookAndFill( h_pre+"dRelPt1p6_PF_vs_d0",   1000, -50, 50, 400, -32, 32, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 1.6) );
	    BookAndFill( h_pre+"dRelPt1p8_Roch_vs_d0", 1000, -50, 50, 400, -16, 16, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 1.8) );
	    BookAndFill( h_pre+"dRelPt1p8_PF_vs_d0",   1000, -50, 50, 400, -16, 16, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 1.8) );
	    BookAndFill( h_pre+"dRelPt2p0_Roch_vs_d0", 1000, -50, 50, 400,  -8,  8, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 2.0) );
	    BookAndFill( h_pre+"dRelPt2p0_PF_vs_d0",   1000, -50, 50, 400,  -8,  8, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 2.0) );
	    BookAndFill( h_pre+"dRelPt2p2_Roch_vs_d0", 1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 2.2) );
	    BookAndFill( h_pre+"dRelPt2p2_PF_vs_d0",   1000, -50, 50, 400,  -4,  4, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 2.2) );
	    BookAndFill( h_pre+"dRelPt2p4_Roch_vs_d0", 1000, -50, 50, 400,  -2,  2, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 2.4) );
	    BookAndFill( h_pre+"dRelPt2p4_PF_vs_d0",   1000, -50, 50, 400,  -2,  2, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 2.4) );
	    BookAndFill( h_pre+"dRelPt2p6_Roch_vs_d0", 1000, -50, 50, 400,  -1,  1, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt_Roch - gen.pt) / pow(gen.pt, 2.6) );
	    BookAndFill( h_pre+"dRelPt2p6_PF_vs_d0",   1000, -50, 50, 400,  -1,  1, 10000*mu.d0_PV*mu.charge, 10000*(mu.pt      - gen.pt) / pow(gen.pt, 2.6) );

	    int eta_sign = gen.eta > 0 ? 1 : -1;
	    BookAndFill( h_pre+"dPhi_vs_d0", 1000, -50, 50, 400, -1, 1, 10000*mu.d0_PV*mu.charge, 1000*(mu.phi - gen.phi)*mu.charge );
	    BookAndFill( h_pre+"dEta_vs_d0", 1000, -50, 50, 400, -1, 1, 10000*mu.d0_PV*mu.charge, 1000*(mu.eta - gen.eta)*mu.charge*eta_sign );
