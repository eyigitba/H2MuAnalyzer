...
const std::vector<std::string> CAT_CUTS = { "noZ_ge2j_btag_mass12",
					    "looseLepMVA_noZ_ge2j_btag_mass12",
					    "medLepMVA_noZ_ge2j_btag_mass12",
					    "tightLepMVA_noZ_ge2j_btag_mass12",
                                            "medLepMVA_onZ_ge2j_btag_mass12" }; // Event selection categories, also applied in parallel
...
void ttH_3l( TString sample = "", TString in_dir = "", TString out_dir = "",
  ...
  for (int iEvt = 0; iEvt < in_chain->GetEntries(); iEvt++) {
	...
	for (int iCat = 0; iCat < CAT_CUTS.size(); iCat++) {
	  ...
	  BookAndFill(tupI, "nJets",      13, -0.5, 12.5, SelectedJets(obj_sel, br).size(),               event_wgt );
	  BookAndFill(tupI, "nBJetsLoose", 6, -0.5,  5.5, SelectedJets(obj_sel, br, "BTagLoose").size(),  event_wgt );
	  BookAndFill(tupI, "nBJetsMed",   5, -0.5,  4.5, SelectedJets(obj_sel, br, "BTagMedium").size(), event_wgt );
	  BookAndFill(tupI, "nBJetsTight", 4, -0.5,  3.5, SelectedJets(obj_sel, br, "BTagTight").size(),  event_wgt );
	  BookAndFill(tupI, "nJetsCent",  11, -0.5, 10.5, SelectedJets(obj_sel, br, "Central").size(),    event_wgt );
	  BookAndFill(tupI, "nJetsFwd",    6, -0.5,  5.5, SelectedJets(obj_sel, br, "Forward").size(),    event_wgt );
