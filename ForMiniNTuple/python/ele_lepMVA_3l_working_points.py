...
def PassLepMVA(lepMVA_cut, mu1_value, mu2_value, lep_value):
    Pass = False
    cut_value = lepMVA_cut
    if mu1_value > cut_value and mu2_value > cut_value and lep_value > 0.4:
    #if lep_value > cut_value:
	Pass = True
    return Pass

def FillHistTerm(histos, term, cut, signals, bkgs, value, Sample_ID, event_wgt):
    lepMVA_cut = "%.2f"%cut
    #blind
	...
	histos[term][lepMVA_cut]["WW"].Fill(value, event_wgt)
	...
	histos[term][lepMVA_cut]["tZq"].Fill(value, event_wgt)
        ...
        histos[term][lepMVA_cut]["ttZ"].Fill(value, event_wgt)
    elif Sample_ID == -0624:
        histos[term][lepMVA_cut]["tW"].Fill(value, event_wgt)
    elif Sample_ID != -062500 and Sample_ID != -062524 and Sample_ID != -06062424 and Sample_ID != -999: #tHq, tHW, and ttWW 
        histos[term][lepMVA_cut]["others"].Fill(value, event_wgt)


def main():
    out_name = "cut_efficiency_with_data_eleMVA_med.root"
...
#    file_chain.Add( file_dir + "bkg.root")
    ...
    terms = ["dimu_mass" ]#"mu1_pt", "mu2_pt", #"mu1_abs_eta", "mu2_abs_eta",
...
#		"0.8","0.82","0.84","0.86","0.88","0.9","0.92","0.94","0.96","0.98"]
    lepMVA_cuts = [ round(1.0*MVA/100 - 1.0 , 2) for MVA in range(200) ]
...
#	    continue
	...
	for cut in lepMVA_cuts:
	    if not PassLepMVA(cut, file_chain.mu1_lepMVA, file_chain.mu2_lepMVA, file_chain.lep_lepMVA):
		continue
	    lepMVA_SF = GetSF("muon", file_chain.mu1_pt, file_chain.mu1_eta, cut) * GetSF("muon", file_chain.mu2_pt, file_chain.mu2_eta, cut) * GetSF("muon", file_chain.lep_pt, file_chain.lep_eta, cut)# "muon" or "ele"
	    if file_chain.Sample_ID==0:
		lepMVA_SF = 1.0

	    #FillHistTerm(histos, "mu1_pt"	, cut, signals, bkgs, file_chain.mu1_pt		, file_chain.Sample_ID	, file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	    #FillHistTerm(histos, "mu2_pt"	, cut, signals, bkgs, file_chain.mu2_pt		, file_chain.Sample_ID  , file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	    FillHistTerm(histos, "dimu_mass"   	, cut, signals, bkgs, file_chain.dimu_mass   	, file_chain.Sample_ID  , file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
            #FillHistTerm(histos, "lep_pt"  , cut, signals, bkgs, file_chain.lep_pt   	, file_chain.Sample_ID	, file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	...
	point_num = 0
        for cut in lepMVA_cuts:
	    lepMVA_cut = "%.2f"%cut
	    if histos["dimu_mass"]["-1.00"][sample].Integral() == 0:
		eff_graphs[sample].SetPoint(point_num, cut, 0)
		point_num += 1
	    else:
	        eff_value = histos["dimu_mass"][lepMVA_cut][sample].Integral() / histos["dimu_mass"]["-1.00"][sample].Integral()
	        eff_graphs[sample].SetPoint(point_num, cut, eff_value)
		point_num += 1
	...
	for sample in signals:
	    ...
	    sig_tot += histos["dimu_mass"]["-1.00"][sample].Integral(range_start, range_end)
	print "lepMVA_cut = " + lepMVA_cut
	print "sig_pass = %f" %sig_pass + "        sig_tot = %f" %sig_tot
	if sig_tot == 0:
	    eff_graphs["sig"].SetPoint(point_num, cut, 0)
	else:
	    eff_graphs["sig"].SetPoint(point_num, cut, sig_pass/sig_tot)

	bkg_pass = 0
	bkg_tot = 0
	for sample in bkgs:
	    #bkg_pass += histos["dimu_mass"][lepMVA_cut][sample].Integral(range_start, range_end)
            bkg_tot += histos["dimu_mass"]["-1.00"][sample].Integral(range_start, range_end)
	    if histos["dimu_mass"]["-1.00"][sample].Integral() != 0:
	        bkg_pass += histos["dimu_mass"][lepMVA_cut][sample].Integral() * histos["dimu_mass"]["-1.00"][sample].Integral(range_start, range_end) / histos["dimu_mass"]["-1.00"][sample].Integral()
	print "lepMVA_cut = " + lepMVA_cut
        print "bkg_pass = %f" %bkg_pass + "        bkg_tot = %f" %bkg_tot
	if bkg_tot == 0:
            eff_graphs["bkg"].SetPoint(point_num, cut, 0)
        else:
            eff_graphs["bkg"].SetPoint(point_num, cut, bkg_pass/bkg_tot)
	if bkg_pass == 0:
	    sens_graph.SetPoint(point_num, cut, 1)
	else:
	    sens_graph.SetPoint(point_num, cut, sig_pass / bkg_pass ** 0.5)
	point_num += 1
