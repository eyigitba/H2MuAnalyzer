...
#		"0.8","0.82","0.84","0.86","0.88","0.9","0.92","0.94","0.96","0.98"]
    lepMVA_cuts = [ "%.2f" %round(1.0*MVA/50 - 1.0 , 2) for MVA in range(100) ]
...
#	    print "event %d" %iEvt
	...
	for lepMVA_cut in lepMVA_cuts:
		...
		continue
	    lepMVA_SF = GetSF("muon", file_chain.mu1_pt, file_chain.mu1_eta, float(lepMVA_cut)) * GetSF("muon", file_chain.mu2_pt, file_chain.mu2_eta, float(lepMVA_cut)) * GetSF("muon", file_chain.lep_pt, file_chain.lep_eta, float(lepMVA_cut))# "muon" or "ele"
	    if file_chain.Sample_ID==0:
		lepMVA_SF = 1.0

	    #FillHistTerm(histos, "mu1_pt"	, lepMVA_cut, signals, bkgs, file_chain.mu1_pt		, file_chain.Sample_ID	, file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	    #FillHistTerm(histos, "mu2_pt"	, lepMVA_cut, signals, bkgs, file_chain.mu2_pt		, file_chain.Sample_ID  , file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	    FillHistTerm(histos, "dimu_mass"   	, lepMVA_cut, signals, bkgs, file_chain.dimu_mass   	, file_chain.Sample_ID  , file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
            #FillHistTerm(histos, "lep_pt"  , lepMVA_cut, signals, bkgs, file_chain.lep_pt   	, file_chain.Sample_ID	, file_chain.xsec_norm * file_chain.event_wgt * lepMVA_SF)
	...
	point_num = 0
        for lepMVA_cut in lepMVA_cuts:
	    if histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral() == 0:
		eff_graphs[sample].SetPoint(point_num, float(lepMVA_cut), 0)
		point_num += 1
	    else:
	        eff_value = histos["dimu_mass"][lepMVA_cut][sample].Integral() / histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral()
	...
	for sample in signals:
	    ...
	    sig_tot += histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral(range_start, range_end)
	...
	for sample in bkgs:
	    #bkg_pass += histos["dimu_mass"][lepMVA_cut][sample].Integral(range_start, range_end)
            bkg_tot += histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral(range_start, range_end)
	    if histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral() != 0:
	        bkg_pass += histos["dimu_mass"][lepMVA_cut][sample].Integral() * histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral(range_start, range_end) / histos["dimu_mass"][lepMVA_cuts[0]][sample].Integral()
	print "lepMVA_cut = " + lepMVA_cut
