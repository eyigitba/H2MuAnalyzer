...
// Command-line options for running in batch.  Running "root -b -l -q macros/Run2_ttH_3l_varStudy.C" will use hard-coded options above.
void Run2_ttH_3l_varStudy( TString myMethodList = "", std::vector<TString> in_dirs = {},
  ...
  // ---------------------------------------------------------------
  std::cout << "\n==> Start Run2_ttH_3l_varStudy" << std::endl;
  ...
  out_file_name.Form( "%s/Run2_ttH_3l_varStudy_ttH_vs_all_jetCuts_2019_07_13_test_v1.root", out_dir.Data() );
  ...
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_resWgt", var_names, var_vals,
  // 					0xfffe, 0xffff, 0xfffffffffffff, "all", "all", "resWgt") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_v1_resWgt", var_names, var_vals,
  // 					0xfffe, 0xffff, 0x7, "all", "all", "resWgt") );

  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars", var_names, var_vals,
  					0xffffe, 0xffffffff, 0xfffffffffffffef, "all", "all", "ge0j") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttZ", var_names, var_vals,
  					0xffffe, 0xffffffff, 0xffbffffffffffef, "ttH", "ttZ", "ge0j") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttW", var_names, var_vals,
  					0xffffe, 0xffffffff, 0xfffffffffffffef, "ttH", "ttW", "ge0j") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttbar", var_names, var_vals,
  // 					0xffffe, 0xffffffff, 0xfffffffffffffef, "ttH", "ttbar", "ge0j") );

  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars", var_names, var_vals,
  					0xffffe, 0xffffffff, 0x03ffff3ffff5cef, "ttH", "all", "le3j_notag") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars", var_names, var_vals,
  					0xffffe, 0xffffffff, 0xfffffffffffffef, "ttH", "all", "ge4j_btag") );


  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars", var_names, var_vals,
  // 					0xffffe, 0xffffffff, 0x03ffff3ffff5cef, "all", "all", "le3j_notag") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttZ", var_names, var_vals,
  // 					0xffffe, 0xffffffff, 0x03ffff3ffff5cef, "ttH", "ttZ", "le3j_notag") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttW", var_names, var_vals,
  // 					0xffffe, 0xffffffff, 0x03ffff3ffff5cef, "ttH", "ttW", "le3j_notag") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttbar_le3j_notag", var_names, var_vals,
  					0xffffe, 0xffffffff, 0x03ffff3ffff5cef, "ttH", "ttbar", "le3j_notag") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_AWB_noMass_allVars_ttH_vs_ttbar_ge4j_btag", var_names, var_vals,
  					0xffffe, 0xffffffff, 0xfffffffffffffef, "ttH", "ttbar", "ge4j_btag") );
  ...
  xwz_vars.push_back( TMVA_var( "muH1_pt_ratio", "muH1_pt_ratio", "", 'F', -88 ) ); //0x10000
  xwz_vars.push_back( TMVA_var( "muH2_pt_ratio", "muH1_pt_ratio", "", 'F', -88 ) ); //0x20000
  ...
  awb_vars.push_back( TMVA_var( "muSS_OS_pair_dPhi_abs", "muSS_OS_pair_dPhi_abs", "", 'F', -88 ) ); //0x0020
  ...
  awb_vars.push_back( TMVA_var( "lep_H_pair_dPhi_abs", "lep_H_pair_dPhi_abs", "", 'F', -88 ) ); //0x10000
  awb_vars.push_back( TMVA_var( "lep_muSS_dPhi_abs",   "lep_muSS_dPhi_abs",   "", 'F', -88 ) ); //0x20000
  awb_vars.push_back( TMVA_var( "lep_muOS_dPhi_abs",   "lep_muOS_dPhi_abs",   "", 'F', -88 ) ); //0x40000
  awb_vars.push_back( TMVA_var( "muSS_muOS_dPhi_abs",  "muSS_muOS_dPhi_abs",  "", 'F', -88 ) ); //0x80000

  awb_vars.push_back( TMVA_var( "HT_jet",       "HT_jet",       "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "HT_lep",       "HT_lep",       "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "HT_jetLep",    "HT_jetLep",    "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "HT_jetLepMHT", "HT_jetLepMHT", "", 'F', -88 ) ); 

  awb_vars.push_back( TMVA_var( "MT_jet",       "MT_jet",       "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "MT_lep",       "MT_lep",       "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "MT_jetLep",    "MT_jetLep",    "", 'F', -88 ) ); 
  awb_vars.push_back( TMVA_var( "MT_jetLepMHT", "MT_jetLepMHT", "", 'F', -88 ) ); 

  awb_vars.push_back( TMVA_var( "min_lep_jet_dR",  "min_lep_jet_dR",  "", 'F', -88 ) );
  awb_vars.push_back( TMVA_var( "min_muSS_jet_dR", "min_muSS_jet_dR", "", 'F', -88 ) );
  awb_vars.push_back( TMVA_var( "min_muOS_jet_dR", "min_muOS_jet_dR", "", 'F', -88 ) );
  awb_vars.push_back( TMVA_var( "avg_jet_dR",      "avg_jet_dR",      "", 'F', -88 ) );


  // Optional extras (targeting ttbar)
  opt_vars.push_back( TMVA_var( "nEles",          "Number of electrons",    "", 'I', -88 ) ); //0x00000001
  opt_vars.push_back( TMVA_var( "muH1_eta_abs",   "|#eta| #mu1 from Higgs", "", 'F', -88 ) ); //0x00000002
  opt_vars.push_back( TMVA_var( "muH2_eta_abs",   "|#eta| #mu2 from Higgs", "", 'F', -88 ) ); //0x00000004
  opt_vars.push_back( TMVA_var( "nJets",          "nJets",                  "", 'I', -88 ) ); //0x00000008

  opt_vars.push_back( TMVA_var( "nJetsCent",   "nJetsCent",   "", 'I', -88 ) ); //0x00000010
  opt_vars.push_back( TMVA_var( "nJetsCent30", "nJetsCent30", "", 'I', -88 ) ); //0x00000020
  opt_vars.push_back( TMVA_var( "nJetsCent40", "nJetsCent40", "", 'I', -88 ) ); //0x00000040
  opt_vars.push_back( TMVA_var( "nJetsCent50", "nJetsCent50", "", 'I', -88 ) ); //0x00000080

  opt_vars.push_back( TMVA_var( "nBJetsLoose", "nBJetsLoose", "", 'I', -88 ) ); //0x00000100
  opt_vars.push_back( TMVA_var( "nBJetsMed",   "nBJetsMed",   "", 'I', -88 ) ); //0x00000200
  opt_vars.push_back( TMVA_var( "nBJetsTight", "nBJetsTight", "", 'I', -88 ) ); //0x00000400
  opt_vars.push_back( TMVA_var( "MASS",        "MASS",        "", 'F', -88 ) ); //0x00000800

  opt_vars.push_back( TMVA_var( "bjet1_pt",  "bjet1_pt",  "", 'F', -88 ) ); //0x00001000
  opt_vars.push_back( TMVA_var( "bjet2_pt",  "bjet2_pt",  "", 'F', -88 ) ); //0x00002000
  opt_vars.push_back( TMVA_var( "bjet1_CSV", "bjet1_CSV", "", 'F', -88 ) ); //0x00004000
  opt_vars.push_back( TMVA_var( "bjet2_CSV", "bjet2_CSV", "", 'F', -88 ) ); //0x00008000

  opt_vars.push_back( TMVA_var( "MAN_topH_b_pt",  "MAN_topH_b_pt",  "", 'F', -88 ) ); //0x00010000
  opt_vars.push_back( TMVA_var( "MAN_topH_b_CSV", "MAN_topH_b_CSV", "", 'F', -88 ) ); //0x00020000
  opt_vars.push_back( TMVA_var( "MAN_W_jet1_pt",  "MAN_W_jet1_pt",  "", 'F', -88 ) ); //0x00040000
  opt_vars.push_back( TMVA_var( "MAN_W_jet2_pt",  "MAN_W_jet2_pt",  "", 'F', -88 ) ); //0x00080000

  opt_vars.push_back( TMVA_var( "bjet1_muSS_dR", "bjet1_muSS_dR", "", 'F', -88 ) ); //0x00100000
  opt_vars.push_back( TMVA_var( "bjet1_lep_dR",  "bjet1_lep_dR",  "", 'F', -88 ) ); //0x00200000
  opt_vars.push_back( TMVA_var( "bjet1_muSS_pt", "bjet1_muSS_pt", "", 'F', -88 ) ); //0x00400000
  opt_vars.push_back( TMVA_var( "bjet1_lep_pt",  "bjet1_lep_pt",  "", 'F', -88 ) ); //0x00800000

  opt_vars.push_back( TMVA_var( "bjet1_muSS_mass", "bjet1_muSS_mass", "", 'F', -88 ) ); //0x01000000
  opt_vars.push_back( TMVA_var( "bjet1_lep_mass",  "bjet1_lep_mass",  "", 'F', -88 ) ); //0x02000000
  opt_vars.push_back( TMVA_var( "MAN_topH_mass",   "MAN_topH_mass",   "", 'F', -88 ) ); //0x04000000
  opt_vars.push_back( TMVA_var( "MAN_topH_pt",     "MAN_topH_pt",     "", 'F', -88 ) ); //0x08000000

  opt_vars.push_back( TMVA_var( "MAN_W_jj_mass",  "MAN_W_jj_mass",  "", 'F', -88 ) ); //0x10000000
  opt_vars.push_back( TMVA_var( "MAN_W_jj_pt",    "MAN_W_jj_pt",    "", 'F', -88 ) ); //0x20000000
  opt_vars.push_back( TMVA_var( "MAN_W_jj_dR",    "MAN_W_jj_dR",    "", 'F', -88 ) ); //0x40000000
  opt_vars.push_back( TMVA_var( "BDT_best_match", "BDT_best_match", "", 'I', -88 ) ); //0x80000000

  opt_vars.push_back( TMVA_var( "BDT_score_jj_blv",  "BDT_score_jj_blv",  "", 'F', -88 ) ); //0x00010000000
  opt_vars.push_back( TMVA_var( "BDT_score_bjj_lv",  "BDT_score_bjj_lv",  "", 'F', -88 ) ); //0x000200000000
  opt_vars.push_back( TMVA_var( "BDT_score_bj_blv",  "BDT_score_bj_blv",  "", 'F', -88 ) ); //0x000400000000
  opt_vars.push_back( TMVA_var( "BDT_score_bjj_blv", "BDT_score_bjj_blv", "", 'F', -88 ) ); //0x000800000000

  opt_vars.push_back( TMVA_var( "BDT_score_max", "BDT_score_max", "", 'F', -88 ) ); //0x001000000000
  opt_vars.push_back( TMVA_var( "BDT_W_jj_mass", "BDT_W_jj_mass", "", 'F', -88 ) ); //0x002000000000
  opt_vars.push_back( TMVA_var( "BDT_W_jj_pt",   "BDT_W_jj_pt",   "", 'F', -88 ) ); //0x004000000000
  opt_vars.push_back( TMVA_var( "BDT_W_jj_dR",   "BDT_W_jj_dR",   "", 'F', -88 ) ); //0x008000000000

  opt_vars.push_back( TMVA_var( "BDT_topL_b_pt",  "BDT_topL_b_pt",  "", 'F', -88 ) ); //0x010000000000
  opt_vars.push_back( TMVA_var( "BDT_topL_b_CSV", "BDT_topL_b_CSV", "", 'F', -88 ) ); //0x020000000000
  opt_vars.push_back( TMVA_var( "BDT_topH_b_pt",  "BDT_topH_b_pt",  "", 'F', -88 ) ); //0x040000000000
  opt_vars.push_back( TMVA_var( "BDT_topH_b_CSV", "BDT_topH_b_CSV", "", 'F', -88 ) ); //0x080000000000

  opt_vars.push_back( TMVA_var( "BDT_W_jet1_pt",  "BDT_W_jet1_pt",   "", 'F', -88 ) ); //0x100000000000
  opt_vars.push_back( TMVA_var( "BDT_W_jet2_pt",  "BDT_W_jet2_pt",   "", 'F', -88 ) ); //0x200000000000
  opt_vars.push_back( TMVA_var( "BDT_topH_mass",  "BDT_topH_mass",   "", 'F', -88 ) ); //0x400000000000
  opt_vars.push_back( TMVA_var( "BDT_topH_pt",    "BDT_topH_pt",     "", 'F', -88 ) ); //0x800000000000

  opt_vars.push_back( TMVA_var( "BDT_lep_topL_b_dR", "BDT_lep_topL_b_dR", "", 'F', -88 ) ); //0x0001000000000000
  opt_vars.push_back( TMVA_var( "BDT_lep_topH_b_dR", "BDT_lep_topH_b_dR", "", 'F', -88 ) ); //0x0002000000000000
  opt_vars.push_back( TMVA_var( "BDT_muH_topL_b_dR", "BDT_muH_topL_b_dR", "", 'F', -88 ) ); //0x0004000000000000
  opt_vars.push_back( TMVA_var( "bjet1_muOS_dR",     "bjet1_muOS_dR",     "", 'F', -88 ) );

  opt_vars.push_back( TMVA_var( "bjet1_muOS_mass", "bjet1_muOS_mass", "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet1_muOS_pt",   "bjet1_muOS_pt",   "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet2_lep_dR",    "bjet2_lep_dR",    "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet2_muSS_dR",   "bjet2_muSS_dR",   "", 'F', -88 ) );

  opt_vars.push_back( TMVA_var( "bjet2_muOS_dR",   "bjet2_muOS_dR",   "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet2_lep_mass",  "bjet2_lep_mass",  "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet2_muSS_mass", "bjet2_muSS_mass", "", 'F', -88 ) );
  opt_vars.push_back( TMVA_var( "bjet2_muOS_mass", "bjet2_muOS_mass", "", 'F', -88 ) );
  ...
  bkg_samps.push_back( std::make_tuple("ttWW",     +16) );
      ...
      if (sig_name.Contains("ttH")   && samp_ID < 0 && !samp_name.Contains("ttH_125")) continue;
      if (bkg_name.Contains("ttW")   && samp_ID > 0 && !samp_name.Contains("ttW"))     continue;
      if (bkg_name.Contains("ttZ")   && samp_ID > 0 && !samp_name.Contains("ttZ"))     continue;
      if (bkg_name.Contains("ttX")   && samp_ID > 0 && (!samp_name.Contains("tZq") && !samp_name.Contains("tZW") &&
							!samp_name.Contains("ttZ") && !samp_name.Contains("ttH") &&
							!samp_name.Contains("tHq") && !samp_name.Contains("tHW") &&
							!samp_name.Contains("ttWW")) ) continue;
      if (bkg_name.Contains("ttbar") && samp_ID > 0 && !samp_name.Contains("tt_ll"))   continue;
      ...
      if (options.Contains("notag") &&  br_int.at("nBJetsLoose") > 1) continue;
      if (options.Contains("btag")  && (br_int.at("nBJetsLoose") < 2 || br_int.at("nBJetsMed") < 1)) continue;
	...
	if (verbose) std::cout << "  * About to fill variable " << vName << " ... ";
	  ...
	  var_vals.at(iVar) = br_int.at("lep_charge");
        ...
        else if ( vName == "BDT_best_match" )
	  var_vals.at(iVar) = br_int.at("BDT_best_match");
        else if ( vName == "BDT_muH_topL_b_dR" )
	  var_vals.at(iVar) = max(br_flt.at("BDT_muH_topL_b_dR"), br_flt.at("BDT_muH_topL_b_dR") / 9);  // Re-scale -99 and -9 to -11 and -1
  ...
  for (int iFact = 0; iFact < factories.size(); iFact++) {
    ...
    if (Use["BDTG_AWB_med2"]) // Slightly heavier, depth 2
      factX->BookMethod( loadX, TMVA::Types::kBDT, "BDTG_AWB_med2", (std::string)
			 "!H:!V:NTrees=100::BoostType=Grad:Shrinkage=0.1:UseBaggedBoost:"+
			 "BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2" );
    
    if (Use["BDTG_AWB_med3"]) // Slightly heavier, depth 3
      factX->BookMethod( loadX, TMVA::Types::kBDT, "BDTG_AWB_med3", (std::string)
			 "!H:!V:NTrees=100::BoostType=Grad:Shrinkage=0.1:UseBaggedBoost:"+
			 "BaggedSampleFraction=0.5:nCuts=20:MaxDepth=3" );
...
int main( int argc, char** argv ) {
  ...
  Run2_ttH_3l_varStudy(methodList);
  return 0;
}
