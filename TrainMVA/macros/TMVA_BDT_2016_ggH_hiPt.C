...
// Options passed in as arguments to TMVA_BDT_2016_ggH_hiPt when running in batch mode
...
const double DIMU_PT_MIN = 150;  // Minimum di-muon pT
// const double LHE_HT_MIN  = 200;  // Minimum LHE HT

using namespace TMVA;


// Command-line options for running in batch.  Running "root -b -l -q macros/TMVA_BDT_2016_ggH_hiPt.C" will use hard-coded options above.
void TMVA_BDT_2016_ggH_hiPt( TString myMethodList = "",
  ...
  // ---------------------------------------------------------------
  std::cout << "\n==> Start TMVA_BDT_2016_ggH_hiPt" << std::endl;
  ...
  out_file_name.Form( "%s/TMVA_BDT_2016_ggH_hiPt_ggH_vs_ZJets_MG_pt150.root", out_dir.Data() );
  ...
  GetSamples2016(samples, "CERN_hiM", "H2Mu_gg" );
  GetSamples2016(samples, "CERN_hiM", "H2Mu_gg_120" );
  GetSamples2016(samples, "CERN_hiM", "H2Mu_gg_130" );
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT100");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT200");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT400");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT600");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT800");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT1200");
  GetSamples2016(samples, "CERN_hiM", "ZJets_MG_HT2500");
  ...
  sig_samps.push_back( std::make_tuple(samples["H2Mu_gg_120"], -11) );
  sig_samps.push_back( std::make_tuple(samples["H2Mu_gg_130"], -21) );
  ...
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_ZJets_MG_pt150", var_names, var_vals,
					0x0650, 0x033c, 0x001e, "all", "all", "ge0j") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v2_ZJets_MG_pt150", var_names, var_vals,
					0x065f, 0x0fff, 0xfff6, "all", "all", "ge0j") );
  ...
  // Muon variables
  mu_vars.push_back( TMVA_var( "mu1_log_pt",    "log_{2}(p_{T}(#mu1))",   "", 'F', -88 ) ); // 0x0001
  mu_vars.push_back( TMVA_var( "mu2_log_pt",    "log_{2}(p_{T}(#mu2))",   "", 'F', -88 ) ); // 0x0002
  mu_vars.push_back( TMVA_var( "mu1_eta",       "|#eta(#mu1)|",           "", 'F', -88 ) ); // 0x0004
  mu_vars.push_back( TMVA_var( "mu2_eta",       "|#eta(#mu2)|",           "", 'F', -88 ) ); // 0x0008

  mu_vars.push_back( TMVA_var( "dimu_log_pt",   "log_{2}(p_{T}(#mu#mu))", "", 'F', -88 ) ); // 0x0010
  ...
  // Jet variables
  jet_vars.push_back( TMVA_var( "jet1_log_pt",     "log_{2}(p_{T}(jet1))", "", 'F', -88 ) ); // 0x0001
  jet_vars.push_back( TMVA_var( "jet2_log_pt",     "log_{2})p_{T}(jet2))", "", 'F', -88 ) ); // 0x0002
  jet_vars.push_back( TMVA_var( "jet1_eta",        "#eta(jet1)",           "", 'F', -88 ) ); // 0x0004
  jet_vars.push_back( TMVA_var( "jet2_eta",        "#eta(jet2)",           "", 'F', -88 ) ); // 0x0008

  jet_vars.push_back( TMVA_var( "dijet1_log_mass", "1^{st} M(jj)",      "GeV", 'F', -88 ) ); // 0x0010
  jet_vars.push_back( TMVA_var( "dijet2_log_mass", "2^{nd} M(jj)",      "GeV", 'F', -88 ) ); // 0x0020
  jet_vars.push_back( TMVA_var( "dijet3_log_mass", "3^{rd} M(jj)",      "GeV", 'F', -88 ) ); // 0x0040
  jet_vars.push_back( TMVA_var( "dijet4_log_mass", "4^{th} M(jj)",      "GeV", 'F', -88 ) ); // 0x0080
  ...
  evt_vars.push_back( TMVA_var( "MET_log",          "log_{2}(MET)",                           "", 'F', -88 ) ); // 0x0010
  evt_vars.push_back( TMVA_var( "MHT_log",          "log_{2}(MHT)",                           "", 'F', -88 ) ); // 0x0020
  evt_vars.push_back( TMVA_var( "MT_had_log",       "log_{2}(M_{T} of jets)",                 "", 'F', -88 ) ); // 0x0040
  evt_vars.push_back( TMVA_var( "mass_had_log",     "log_{2}(Mass of jets)",                  "", 'F', -88 ) ); // 0x0080

  evt_vars.push_back( TMVA_var( "dimu_jet1_dPhi", "log_{2}[1 + cos(|d#phi(#mu#mu, jet1)|)]",  "", 'F', -88 ) ); // 0x0100
  evt_vars.push_back( TMVA_var( "dimu_jet2_dPhi", "log_{2}[1 + cos(|d#phi(#mu#mu, jet2)|)]",  "", 'F', -88 ) ); // 0x0200
  evt_vars.push_back( TMVA_var( "dimu_jet1_dEta", "[#eta(#mu#mu) - #eta(jet1)]*sign(#eta)",   "", 'F', -88 ) ); // 0x0400
  evt_vars.push_back( TMVA_var( "dimu_jet2_dEta", "[#eta(#mu#mu) - #eta(jet2)]*sign(#eta)",   "", 'F', -88 ) ); // 0x0800

  evt_vars.push_back( TMVA_var( "dimu_jet1_dR",      "#DeltaR(#mu#mu, jet1)",                 "", 'F', -88 ) ); // 0x1000
  evt_vars.push_back( TMVA_var( "dimu_jet2_dR",      "#DeltaR(#mu#mu, jet2)",                 "", 'F', -88 ) ); // 0x2000
  evt_vars.push_back( TMVA_var( "dimu_jet1_ptRatio", "p_{T}(jet1) / p_{T}(#mu#mu)",           "", 'F', -88 ) ); // 0x4000
  evt_vars.push_back( TMVA_var( "dimu_jet2_ptRatio", "p_{T}(jet2) / p_{T}(#mu#mu)",           "", 'F', -88 ) ); // 0x8000
  ...
  spec_vars.push_back( TMVA_var( "LHE_HT",    "LHE H_{T}",         "GeV", 'F', -77 ) );
  ...
  for (int iSamp = 0; iSamp < all_samps.size(); iSamp++) {
    ...
    for (int i = 0; i < samp->filenames.size(); i++) {
      ...
      std::cout << " with name " << samp->filenames.at(i) << std::endl;
	...
	continue;
      // Pre-select high pT di-muon pairs
      if ( candPt < DIMU_PT_MIN )
	continue;
      ...
      int            etaSign  = ( candPair.eta > 0 ? 1 : -1);
      TLorentzVector dimu_vec = FourVec(candPair, obj_sel.mu_pt_corr);
      TLorentzVector jet1_vec;
      TLorentzVector jet2_vec;
      if (nJets > 0) jet1_vec = FourVec(jets.at(0));
      if (nJets > 1) jet2_vec = FourVec(jets.at(1));
	...
	for (int iVar = 0; iVar < var_names.size(); iVar++) {
	  ...
	  /////////////////////////////////////////////////////
	    ...
	    // Muon variables
	    if      (vName == "mu1_log_pt") var_vals.at(iVar) = log2( MuonPt(mu1, obj_sel.mu_pt_corr) );
	    else if (vName == "mu2_log_pt") var_vals.at(iVar) = log2( MuonPt(mu2, obj_sel.mu_pt_corr) );
	    else if (vName == "mu1_eta")    var_vals.at(iVar) = mu1.eta * etaSign;
	    else if (vName == "mu2_eta")    var_vals.at(iVar) = mu2.eta * etaSign;
	    
	    else if (vName == "dimu_log_pt") var_vals.at(iVar) = log2( MuPairPt(candPair, obj_sel.mu_pt_corr) );
	    else if (vName == "dimu_dMass")  var_vals.at(iVar) = MuPairMassErr(candPair, obj_sel.mu_pt_corr);
	    else if (vName == "dimu_eta")    var_vals.at(iVar) = abs(candPair.eta);
	    ...
	    // Jet variables
	    else if (vName == "jet1_log_pt") var_vals.at(iVar) = (nJets > 0 ? log2(jets.at(0).pt)      :  4);
	    else if (vName == "jet2_log_pt") var_vals.at(iVar) = (nJets > 1 ? log2(jets.at(1).pt)      :  4);
	    else if (vName == "jet1_eta")    var_vals.at(iVar) = (nJets > 0 ? jets.at(0).eta * etaSign : -6);
	    else if (vName == "jet2_eta")    var_vals.at(iVar) = (nJets > 1 ? jets.at(1).eta * etaSign : -6);
	    
	    else if (vName == "dijet1_log_mass") var_vals.at(iVar) = (nJetPairs > 0 ? log2(dijets.at(0).mass) : 4);
	    else if (vName == "dijet2_log_mass") var_vals.at(iVar) = (nJetPairs > 1 ? log2(dijets.at(1).mass) : 4);
	    else if (vName == "dijet3_log_mass") var_vals.at(iVar) = (nJetPairs > 2 ? log2(dijets.at(2).mass) : 4);
	    else if (vName == "dijet4_log_mass") var_vals.at(iVar) = (nJetPairs > 3 ? log2(dijets.at(3).mass) : 4);
	    
	    else if (vName == "dijet1_abs_dEta") var_vals.at(iVar) = (nJetPairs > 0 ? abs(dijets.at(0).dEta) : -1);
	    else if (vName == "dijet2_abs_dEta") var_vals.at(iVar) = (nJetPairs > 1 ? abs(dijets.at(1).dEta) : -1);
	    else if (vName == "dijet3_abs_dEta") var_vals.at(iVar) = (nJetPairs > 2 ? abs(dijets.at(2).dEta) : -1);
	    else if (vName == "dijet4_abs_dEta") var_vals.at(iVar) = (nJetPairs > 3 ? abs(dijets.at(3).dEta) : -1);
	    ...
	    else if (vName == "MET_log")      var_vals.at(iVar) = log2( std::fmax(MET,              0.5) );
	    else if (vName == "MHT_log")      var_vals.at(iVar) = log2( std::fmax(br.mht->pt,       0.5) );
	    else if (vName == "MT_had_log")   var_vals.at(iVar) = log2( std::fmax(br.mht->MT_had,   0.5) );
	    else if (vName == "mass_had_log") var_vals.at(iVar) = log2( std::fmax(br.mht->mass_had, 0.5) );

	    else if (vName == "dimu_jet1_dPhi") var_vals.at(iVar) = ( nJets > 0 ? log2( 1 + cos(dimu_vec.DeltaPhi(jet1_vec)) ) : -30 );
	    else if (vName == "dimu_jet2_dPhi") var_vals.at(iVar) = ( nJets > 1 ? log2( 1 + cos(dimu_vec.DeltaPhi(jet2_vec)) ) : -30 );
	    else if (vName == "dimu_jet1_dEta") var_vals.at(iVar) = ( nJets > 0 ? ( candPair.eta - jet1_vec.Eta() ) * etaSign  :  -7 );
	    else if (vName == "dimu_jet2_dEta") var_vals.at(iVar) = ( nJets > 1 ? ( candPair.eta - jet2_vec.Eta() ) * etaSign  :  -7 );
	    
	    else if (vName == "dimu_jet1_dR")      var_vals.at(iVar) = ( nJets > 0 ? dimu_vec.DeltaR(jet1_vec)  : -1 );
	    else if (vName == "dimu_jet2_dR")      var_vals.at(iVar) = ( nJets > 1 ? dimu_vec.DeltaR(jet2_vec)  : -1 );
	    else if (vName == "dimu_jet1_ptRatio") var_vals.at(iVar) = ( nJets > 0 ? jet1_vec.Pt() / candPt     :  0 );
	    else if (vName == "dimu_jet2_ptRatio") var_vals.at(iVar) = ( nJets > 1 ? jet2_vec.Pt() / candPt     :  0 );
...
int main( int argc, char** argv ) {
  ...
  TMVA_BDT_2016_ggH_hiPt(methodList);
  return 0;
}
