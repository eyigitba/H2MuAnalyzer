...
const TString IN_DIR  = "/afs/cern.ch/work/a/abrinke1/public/H2Mu/2017/Histograms/WH_lep_AWB_2019_05_16_v1/files/";
...
const std::vector<TString> CATS = {"OPT_e2mu_CAT_looseLepMVA_noBtag_mass12",
				   "OPT_3mu_CAT_looseLepMVA_noBtag_mass12"};


using namespace TMVA;


// Command-line options for running in batch.  Running "root -b -l -q macros/TMVA_BDT_2017_WH_lep.C" will use hard-coded options above.
void TMVA_BDT_2017_WH_lep( TString myMethodList = "", TString in_dir = "",
  ...
  // ---------------------------------------------------------------
  std::cout << "\n==> Start TMVA_BDT_2017_WH_lep" << std::endl;
  ...
  out_file_name.Form( "%s/TMVA_BDT_2017_WH_lep_all_vs_all_v2.root", out_dir.Data() );
  ...
  sig_samps.push_back( std::make_tuple("H2Mu_ttH_135",    -26) );
  ...
  bkg_samps.push_back( std::make_tuple("ttZ_loM", +12) );
  ...
  for (int i = 0; i < in_files.size(); i++) {
    TString in_file = in_dir+in_files.at(i);
    // Open input file
    TFile *file_tmp(0);
  ...
  for (int iEvt = 0; iEvt < nEvents; iEvt++) {
    ...
    for (int iSamp = 0; iSamp < all_samps.size(); iSamp++) {
      ...
      int     iSamp_ID   = std::get<1>(all_samps.at(iSamp));
    ...
    for (int iFact = 0; iFact < factories.size(); iFact++) {
      ...
      if (sig_name.Contains("e2mu") && samp_ID < 0 && br_int.at("OPT_e2mu") != 1) continue;
      if (bkg_name.Contains("e2mu") && samp_ID > 0 && br_int.at("OPT_e2mu") != 1) continue;
      if (sig_name.Contains("3mu")  && samp_ID < 0 && br_int.at("OPT_3mu")  != 1) continue;
      if (bkg_name.Contains("3mu")  && samp_ID > 0 && br_int.at("OPT_3mu")  != 1) continue;
      ...
      if (br_int.at("OPT_3mu") == 1 && abs(br_flt.at("OS_pair_mass") - 91) < 5) continue;
	...
	if (verbose) std::cout << "  * About to fill variable " << vName << " ... ";
	  ...
	  var_vals.at(iVar) = br_int.at("OPT_e2mu");
	...
	else var_vals.at(iVar) = br_flt.at(vName);
...
int main( int argc, char** argv ) {
  ...
  TMVA_BDT_2017_WH_lep(methodList);
  return 0;
}
