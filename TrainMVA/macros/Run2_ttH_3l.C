...
// Command-line options for running in batch.  Running "root -b -l -q macros/Run2_ttH_3l.C" will use hard-coded options above.
void Run2_ttH_3l( TString myMethodList = "", std::vector<TString> in_dirs = {},
  ...
  out_file_name.Form( "%s/Run2_ttH_3l_all_vs_all_2019_07_04_v2.root", out_dir.Data() );
  ...
  // Lepton variables        : 0x15026  / 0x35026
  // Jet and MET variables   : 0x000015 / 0x330037
  // ttH reconstr. variables : 0x1600   / 0x3f10

  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsT_resWgt", var_names, var_vals,
  					0x150e6, 0x000015, 0x1600, "all", "all", "resWgt") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsT_noBDT_resWgt", var_names, var_vals,
  					0x150e6, 0x000015, 0x0000, "all", "all", "resWgt") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsM_resWgt", var_names, var_vals,
  					0x350e6, 0x330037, 0x3f10, "all", "all", "resWgt") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsM_noBDT_resWgt", var_names, var_vals,
  					0x350e6, 0x330037, 0x0000, "all", "all", "resWgt") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsAll_resWgt", var_names, var_vals,
  					0xffffe, 0xffffff, 0xffff, "all", "all", "resWgt") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_2019_07_04_varsAll_withMass", var_names, var_vals,
  // 					0xfffff, 0xffffff, 0xffff, "all", "all", "withMass") );
  ...
  std::vector<TMVA_var> lep_vars;   // Variables originally from Xunwu
  std::vector<TMVA_var> jet_vars;   // Variables introduced by Andrew
  std::vector<TMVA_var> rec_vars;   // Optional extras
  ...
  // Lepton variables : 0x15026 / 0x35026
  lep_vars.push_back( TMVA_var( "H_pair_mass",  "H_pair_mass",  "", 'F', -88 ) ); //0x0001
  lep_vars.push_back( TMVA_var( "H_pair_pt",    "H_pair_pt",    "", 'F', -88 ) ); //0x0002 **
  lep_vars.push_back( TMVA_var( "OS_pair_mass", "OS_pair_mass", "", 'F', -88 ) ); //0x0004 **
  lep_vars.push_back( TMVA_var( "OS_pair_pt",   "OS_pair_pt",   "", 'F', -88 ) ); //0x0008

  lep_vars.push_back( TMVA_var( "lep_charge",   "lep_charge",             "", 'I', -88 ) ); //0x0010
  lep_vars.push_back( TMVA_var( "nEles",        "Number of electrons",    "", 'I', -88 ) ); //0x0020 **
  lep_vars.push_back( TMVA_var( "muH1_eta_abs", "|#eta| #mu1 from Higgs", "", 'F', -88 ) ); //0x0040
  lep_vars.push_back( TMVA_var( "muH2_eta_abs", "|#eta| #mu2 from Higgs", "", 'F', -88 ) ); //0x0080

  lep_vars.push_back( TMVA_var( "lep_H_pair_dEta", "lep_H_pair_dEta", "", 'F', -88 ) ); //0x0100
  lep_vars.push_back( TMVA_var( "lep_H_pair_dR",   "lep_H_pair_dR",   "", 'F', -88 ) ); //0x0200
  lep_vars.push_back( TMVA_var( "lep_muOS_dEta",  "lep_muOS_dEta",    "", 'F', -88 ) ); //0x0400
  lep_vars.push_back( TMVA_var( "lep_muOS_dR",    "lep_muOS_dR",      "", 'F', -88 ) ); //0x0800

  lep_vars.push_back( TMVA_var( "muSS_muOS_dR",      "muSS_muOS_dR",      "", 'F', -88 ) ); //0x1000 **
  lep_vars.push_back( TMVA_var( "muSS_muOS_dEta",    "muSS_muOS_dEta",    "", 'F', -88 ) ); //0x2000
  lep_vars.push_back( TMVA_var( "muSS_OS_pair_dR",   "muSS_OS_pair_dR",   "", 'F', -88 ) ); //0x4000 **
  lep_vars.push_back( TMVA_var( "muSS_OS_pair_dEta", "muSS_OS_pair_dEta", "", 'F', -88 ) ); //0x8000

  lep_vars.push_back( TMVA_var( "lep_pt",  "lep_pt",  "", 'F', -88 ) ); //0x10000 **
  lep_vars.push_back( TMVA_var( "muSS_pt", "muSS_pt", "", 'F', -88 ) ); //0x20000 *


  // Jet and MET variables : 0x000015 / 0x330037
  jet_vars.push_back( TMVA_var( "nJetsCent",   "nJetsCent",   "", 'I', -88 ) ); //0x0001 **
  jet_vars.push_back( TMVA_var( "nJetsCent30", "nJetsCent30", "", 'I', -88 ) ); //0x0002 *
  jet_vars.push_back( TMVA_var( "nJetsCent40", "nJetsCent40", "", 'I', -88 ) ); //0x0004 **
  jet_vars.push_back( TMVA_var( "nJetsCent50", "nJetsCent50", "", 'I', -88 ) ); //0x0008

  jet_vars.push_back( TMVA_var( "nBJetsLoose", "nBJetsLoose", "", 'I', -88 ) ); //0x0010 **
  jet_vars.push_back( TMVA_var( "nBJetsMed",   "nBJetsMed",   "", 'I', -88 ) ); //0x0020 *
  jet_vars.push_back( TMVA_var( "nBJetsTight", "nBJetsTight", "", 'I', -88 ) ); //0x0040
  jet_vars.push_back( TMVA_var( "MASS",        "MASS",        "", 'F', -88 ) ); //0x0080

  jet_vars.push_back( TMVA_var( "MET",           "MET",           "", 'F', -88 ) ); //0x0100
  jet_vars.push_back( TMVA_var( "MHT",           "MHT",           "", 'F', -88 ) ); //0x0200
  jet_vars.push_back( TMVA_var( "bjet1_muSS_dR", "bjet1_muSS_dR", "", 'F', -88 ) ); //0x0400
  jet_vars.push_back( TMVA_var( "bjet1_lep_dR",  "bjet1_lep_dR",  "", 'F', -88 ) ); //0x0800

  jet_vars.push_back( TMVA_var( "bjet1_muSS_pt",   "bjet1_muSS_pt",   "", 'F', -88 ) ); //0x1000
  jet_vars.push_back( TMVA_var( "bjet1_lep_pt",    "bjet1_lep_pt",    "", 'F', -88 ) ); //0x2000
  jet_vars.push_back( TMVA_var( "bjet1_muSS_mass", "bjet1_muSS_mass", "", 'F', -88 ) ); //0x4000
  jet_vars.push_back( TMVA_var( "bjet1_lep_mass",  "bjet1_lep_mass",  "", 'F', -88 ) ); //0x8000

  jet_vars.push_back( TMVA_var( "lep_MET_MT",       "lep_MET_MT",         "", 'F', -88 ) ); //0x00010000 *
  jet_vars.push_back( TMVA_var( "lep_MHT_MT",       "lep_MHT_MT",         "", 'F', -88 ) ); //0x00020000 *
  jet_vars.push_back( TMVA_var( "lep_MET_dPhi_abs", "lep_MET_dPhi_abs",   "", 'F', -88 ) ); //0x00040000
  jet_vars.push_back( TMVA_var( "lep_MHT_dPhi_abs",  "lep_MHT_dPhi_abs",  "", 'F', -88 ) ); //0x00080000

  jet_vars.push_back( TMVA_var( "muSS_MET_MT",       "muSS_MET_MT",       "", 'F', -88 ) ); //0x00100000 *
  jet_vars.push_back( TMVA_var( "muSS_MHT_MT",       "muSS_MHT_MT",       "", 'F', -88 ) ); //0x00200000 *
  jet_vars.push_back( TMVA_var( "muSS_MET_dPhi_abs", "muSS_MET_dPhi_abs", "", 'F', -88 ) ); //0x00400000
  jet_vars.push_back( TMVA_var( "muSS_MHT_dPhi_abs", "muSS_MHT_dPhi_abs", "", 'F', -88 ) ); //0x00800000


  // ttH reconstr. variables : 0x1600 / 0x3f10
  rec_vars.push_back( TMVA_var( "MAN_W_jet2_pt",  "MAN_W_jet2_pt",  "", 'F', -88 ) ); //0x0001
  rec_vars.push_back( TMVA_var( "MAN_W_jj_mass",  "MAN_W_jj_mass",  "", 'F', -88 ) ); //0x0002
  rec_vars.push_back( TMVA_var( "MAN_W_jj_pt",    "MAN_W_jj_pt",    "", 'F', -88 ) ); //0x0004
  rec_vars.push_back( TMVA_var( "MAN_W_jj_dR",    "MAN_W_jj_dR",    "", 'F', -88 ) ); //0x0008

  rec_vars.push_back( TMVA_var( "BDT_W_jj_mass", "BDT_W_jj_mass", "", 'F', -88 ) ); //0x0010 *
  rec_vars.push_back( TMVA_var( "BDT_W_jj_pt",   "BDT_W_jj_pt",   "", 'F', -88 ) ); //0x0020
  rec_vars.push_back( TMVA_var( "BDT_topH_mass", "BDT_topH_mass", "", 'F', -88 ) ); //0x0040
  rec_vars.push_back( TMVA_var( "BDT_topH_pt",   "BDT_topH_pt",   "", 'F', -88 ) ); //0x0080

  rec_vars.push_back( TMVA_var( "BDT_score_jj_blv",  "BDT_score_jj_blv",  "", 'F', -88 ) ); //0x0100 *
  rec_vars.push_back( TMVA_var( "BDT_score_bjj_lv",  "BDT_score_bjj_lv",  "", 'F', -88 ) ); //0x0200 **
  rec_vars.push_back( TMVA_var( "BDT_score_bj_blv",  "BDT_score_bj_blv",  "", 'F', -88 ) ); //0x0400 **
  rec_vars.push_back( TMVA_var( "BDT_score_bjj_blv", "BDT_score_bjj_blv", "", 'F', -88 ) ); //0x0800 *

  rec_vars.push_back( TMVA_var( "BDT_score_max",  "BDT_score_max",  "", 'F', -88 ) ); //0x1000 **
  rec_vars.push_back( TMVA_var( "BDT_best_match", "BDT_best_match", "", 'I', -88 ) ); //0x2000 *
  ...
  for (int iFact = 0; iFact < factories.size(); iFact++) {
    ...
    std::cout << "*** Lepton variables ***" << std::endl;
    for (int i = 0; i < lep_vars.size(); i++) {
      if ( 0x1 & (std::get<5>(factories.at(iFact)) >> i) ) { // Hex bit mask for lep_vars
    ...
    std::cout << "*** Jet and MET variables ***" << std::endl;
    ...
    std::cout << "*** ttH reconstruction variables ***" << std::endl;
    for (int i = 0; i < rec_vars.size(); i++) {
      if ( 0x1 & (std::get<7>(factories.at(iFact)) >> i) ) { // Hex bit mask for rec_vars
	TMVA_var v = rec_vars.at(i);
	std::cout << v.name << std::endl;
	std::get<1>(factories.at(iFact))->AddVariable( v.name, v.descr, v.unit, v.type ); // Add var to dataloader
	std::get<3>(factories.at(iFact)).push_back( v.name );    // Add to vector of var names
	std::get<4>(factories.at(iFact)).push_back( v.def_val ); // Add to vector of var values
