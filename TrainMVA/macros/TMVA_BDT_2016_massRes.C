...
// Options passed in as arguments to TMVA_BDT_2016_massRes when running in batch mode
...
const TString OUT_DIR  = "/afs/cern.ch/work/a/abrinke1/public/H2Mu/2016/H2MuAnalyzer/TrainMVA/output";
...
// Command-line options for running in batch.  Running "root -b -l -q macros/TMVA_BDT_2016_massRes.C" will use hard-coded options above.
void TMVA_BDT_2016_massRes( TString myMethodList = "",
  ...
  // ---------------------------------------------------------------
  std::cout << "\n==> Start TMVA_BDT_2016_massRes" << std::endl;
  ...
  out_file_name.Form( "%s/2018_11_16_TMVA_BDT_2016_massRes_ggH_vs_Z_AMC_MG.root", out_dir.Data() );
  ...
  // GetSamples2016(samples, "CERN_hiM", "ZJets_AMC_0j");
  // GetSamples2016(samples, "CERN_hiM", "ZJets_AMC_1j");
  // GetSamples2016(samples, "CERN_hiM", "ZJets_AMC_2j");
  ...
  bkg_samps.push_back( std::make_tuple(samples["ZJets_AMC_0j"], + 2) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_AMC_1j"], + 3) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_AMC_2j"], + 4) );

  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG"],              + 11) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_70_100"],    + 12) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_100_200"],   + 13) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_200_400"],   + 14) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_400_600"],   + 15) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_600_800"],   + 16) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_800_1200"],  + 17) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_1200_2500"], + 18) );
  bkg_samps.push_back( std::make_tuple(samples["ZJets_MG_HT_2500_inf"],  + 19) );
  ...
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_etas", var_names, var_vals,
					0x065c, 0x033c, 0x001e, "all", "all", "ge0j") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_mass_wgt_etas", var_names, var_vals,
  // 					0x065c, 0x033c, 0x001e, "all", "all", "ge0j") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_res_wgt_etas", var_names, var_vals,
					0x065c, 0x033c, 0x001e, "all", "all", "ge0j") );
  // factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_mass_wgt_etas_res", var_names, var_vals,
  // 					0x067c, 0x033c, 0x001e, "all", "all", "ge0j") );
  factories.push_back( std::make_tuple( nullF, nullL, "f_Opt_v1_res_wgt_etas_res", var_names, var_vals,
					0x067c, 0x033c, 0x001e, "all", "all", "ge0j") );
  ...
  spec_vars.push_back( TMVA_var( "mass_wgt",  "Signal mass weight",    "", 'F', -77 ) );
  spec_vars.push_back( TMVA_var( "res_wgt",   "Signal res. weight",    "", 'F', -77 ) );
  ...
  for (int iSamp = 0; iSamp < all_samps.size(); iSamp++) {
	...
	std::cout << "  * nEvt_fail_presel = " << nEvt_fail_presel << std::endl;
      }
	...
	continue;
      ...
      float candMassErr = MuPairMassErr(candPair, obj_sel.mu_pt_corr) * 2;  // massErr in NTuples is off by a factor of 2
	...
	samp_wgt *= (1.0 * presc / DAT_PRESC);
      ...
      double mass_wgt = tripGaus->Eval(candMass) / tripGausNorm;
      double res_wgt  = (candMass / candMassErr) / 58.58;
	...
	TString fact_name = std::get<2>(factories.at(iFact));
	...
	for (int iVar = 0; iVar < var_names.size(); iVar++) {
	  ...
	  else if ( vName == "mass_wgt" )
	    var_vals.at(iVar) = mass_wgt;
	  ...
	  /////////////////////////////////////////////////////
	    ...
	    else if (vName == "dimu_pt")    var_vals.at(iVar) = candPt;
	    else if (vName == "dimu_dMass") var_vals.at(iVar) = candMassErr;
	...
	if (samp_ID < 0) {
	  if (fact_name.Contains("mass_wgt")) sig_evt_weight *= mass_wgt;
	  if (fact_name.Contains("res_wgt"))  sig_evt_weight *= res_wgt;
	}
...
int main( int argc, char** argv ) {
  ...
  TMVA_BDT_2016_massRes(methodList);
  return 0;
}
