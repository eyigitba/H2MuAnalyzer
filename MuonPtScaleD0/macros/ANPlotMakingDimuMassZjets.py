...
OUTDIR = 'plots_DY_data_dimu_mass_%s' % YEAR
...
def main():
    ...
    # dimu_mass_Roch = in_file.Get('sum_mass_peaks/ZJets_AMCRochall').Clone()
    # dimu_mass_geofit = in_file.Get('sum_mass_peaks/ZJets_AMCGeoBSRochall').Clone()

    dimu_mass_Roch = in_file.Get('sum_mass_peaks/dataRochall').Clone()
    dimu_mass_geofit = in_file.Get('sum_mass_peaks/dataGeoBSRochall').Clone()
    ...
    cms_latex.SetTextAlign(11)
    cms_latex.SetTextSize(0.03)
    cms_latex.DrawLatexNDC(0.15, 0.94, '#scale[1.5]{CMS}')
    cms_latex.DrawLatexNDC(0.24, 0.94,'#font[52]{#scale[1.2]{preliminary}}')
    # cms_latex.DrawLatexNDC(0.82, 0.94,'#scale[1.3]{%s}' % YEAR)
    cms_latex.DrawLatexNDC(0.72, 0.91,'#font[42]{59.7 fb^{-1} (13 TeV)}') #35.9 41.5 59.7
