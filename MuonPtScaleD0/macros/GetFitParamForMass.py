...
INDIR = 'plots_fineBinned_fullD0Range_0p002_%s' % YEAR

INFILE = 'FindPeaks.root'

OUTFILE = 'fit_param_fullD0Range_0p002_%s.txt' % YEAR
        ...
        for corr in ['Roch', 'KaMu', '']:
            ...
            for eta in ['eta_0_0p9', 'eta_0p9_1p7', 'eta_1p7_inf']: # |eta| binned
                ...
                # for pt in ['inclusive', 'pt_20_35', 'pt_35_42', 'pt_42_50', 'pt_50_inf']:
                    ...
                    fl = graph.GetFunction("fl_%s" % g_str)
                    fr = graph.GetFunction("fr_%s" % g_str)
                    ...
                    # out_file.write('intercept : ' + str(f.GetParameter(0)) + '\pm' + str(f.GetParError(0)) + ' \n')
                    # out_file.write('intercept_err : ' + str(f.GetParError(0)) + '\n')
                    out_file.write('slope : ' + str(round(f.GetParameter(1),2)) + '\n')
                    # out_file.write('slope : ' + str(f.GetParameter(1)) + '\pm' + str(f.GetParError(1)) + '\n')
                    # out_file.write('slope_err : ' + str(f.GetParError(1)) + '\n\n')


                    out_file.write('\nfl_%s : \n' % g_str)
                    # out_file.write('intercept : ' + str(fl.GetParameter(0)) + '\pm' + str(fl.GetParError(0)) + ' \n')
                    out_file.write('intercept : ' + str(round(fl.GetParameter(0),4)) + ' \n')
                    # out_file.write('intercept_err : ' + str(f.GetParError(0)) + '\n')
                    # out_file.write('slope : ' + str(fl.GetParameter(1)) + '\pm' + str(fl.GetParError(1)) + '\n')
                    out_file.write('slope : ' + str(round(fl.GetParameter(1),2)) + '\n')
                    # out_file.write('slope_err : ' + str(f.GetParError(1)) + '\n\n')



                    out_file.write('\nfr_%s : \n' % g_str)
                    # out_file.write('intercept : ' + str(fr.GetParameter(0)) + '\pm' + str(fr.GetParError(0)) + ' \n')
                    out_file.write('intercept : ' + str(round(fr.GetParameter(0),4)) + ' \n')
                    # out_file.write('intercept_err : ' + str(f.GetParError(0)) + '\n')
                    # out_file.write('slope : ' + str(fr.GetParameter(1)) + '\pm' + str(fr.GetParError(1)) + '\n')
                    out_file.write('slope : ' + str(round(fr.GetParameter(1),2)) + '\n')
                    # out_file.write('slope_err : ' + str(f.GetParError(1)) + '\n\n')
