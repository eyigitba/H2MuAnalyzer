...
def poly(x_var, order, x_min, x_max):
    print '\nDefining polynomial model of order %d with x_min = %f, x_max = %f' % (order, x_min, x_max)
    coef      = []  ## Set of all coefficients in the polynomial
    model_str = ''  ## String defining the linear model
    ...
    for i in range(order):
        coef.append( RooRealVar('c%d' % i, 'c%d' % i, 0) )
        arg_list.add(coef[i])
        model_str += '+@%d*pow((@0 - %f)/%f, %d)' % (i+1, x_min, x_range, i)

    print '  * Final model string: %s' % model_str
    poly_model = RooGenericPdf('poly_model', model_str[1:], arg_list)
    return poly_model, coef
